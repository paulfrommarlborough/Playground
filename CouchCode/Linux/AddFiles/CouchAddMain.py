...
def main():
    ...
    if ci.operations_list is None:
        print('No operations list..')
        return

    for jdata in ci.operations_list:           
        #print(f'Couch Record ADD: Add Entry, {jdata["name"]}, {jdata["os"]}, {jdata["ip"]}, {jdata["date"]} ')
        ...
        if jdata["zip"] is None:
            ...
            gdf  = GetDataFileUNIX(jdata["name"], jdata["os"], jdata["date"], ci.work_dir, ci.root_dir)
